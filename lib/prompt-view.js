...
let pug = null;
const PromptViewList = require('./prompt-view-list');
...
function initialize() {
  ...
  pugDirectory = `${atom.configDirPath}/packages/user-support-helper/pug`;

  allowUnsafeEval(() => {
    allowUnsafeNewFunction(() => {
      pug = require('pug');
      templates.promptView = pug.compileFile(`${pugDirectory}/prompt-view.pug`);
      templates.promptViewDropdown = pug.compileFile(`${pugDirectory}/prompt-view-dropdown.pug`);
      templates.promptViewCheckbox = pug.compileFile(`${pugDirectory}/prompt-view-checkbox.pug`);
    })
  });
  inputInterfaces.input = (options) => {
    ...
    return {
      ...
      onChange: (callback) => {
        // TODO should use addEventListener
        editorElement.onkeyup = callback;
      }
    };
  };
  inputInterfaces.list = (options) => {
    let view = new PromptViewList();
...
class PromptView {
  ...
  updateInput(defaultValue, choices) {
    ...
    for (let i = 0; i < l; i++) contentDom.removeChild(contentDom.children[0])
